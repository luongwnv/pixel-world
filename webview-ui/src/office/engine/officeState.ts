...
export class OfficeState {
  ...
  /** Reassign user character to a desk seat (typing) or non-desk seat (reading/idle).
   *  Forces the character to walk to the new seat. */
  reassignUserSeat(id: number, preferDesk: boolean): void {
    const ch = this.characters.get(id);
    if (!ch) return;
    ...
    for (const [uid, seat] of this.seats) {
      seatInfo.push(`${uid}: nearDesk=${seat.nearDesk}, assigned=${seat.assigned}`);
    }
    console.log(`[reassignUserSeat] id=${id} preferDesk=${preferDesk} currentSeat=${ch.seatId}`);
    console.log(`[reassignUserSeat] seats:`, seatInfo.join(' | '));

    // Find the best seat matching preference
    let bestSeatId: string | null = null;

    // First: find an unassigned seat matching preference
    for (const [uid, seat] of this.seats) {
      if (seat.assigned) continue;
      if (seat.nearDesk === preferDesk) {
        bestSeatId = uid;
        break;
      }
    }

    // Second: allow the character's own seat if it matches
    if (!bestSeatId && ch.seatId) {
      const ownSeat = this.seats.get(ch.seatId);
      if (ownSeat && ownSeat.nearDesk === preferDesk) {
        bestSeatId = ch.seatId;
      }
    }
    ...
    if (!bestSeatId) {
      for (const [uid, seat] of this.seats) {
        if (!seat.assigned || uid === ch.seatId) {
          bestSeatId = uid;
          break;
        }
      }
    ...
    console.log(`[reassignUserSeat] bestSeatId=${bestSeatId}`);

    // Reassign if different seat found
    if (bestSeatId && bestSeatId !== ch.seatId) {
      // Free old seat
      if (ch.seatId) {
      ...
      // Assign new seat
      const newSeat = this.seats.get(bestSeatId)!;
      newSeat.assigned = true;
      ch.seatId = bestSeatId;
    }

    // Force character to stand up and walk to (new) seat
