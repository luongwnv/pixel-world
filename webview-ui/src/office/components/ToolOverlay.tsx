  ...
  return (
    <>
      {allIds.map((id) => {
        ...
        const isCopilot = copilotAgentIds.has(id);
        const isUser = userAgentIds.has(id);
        let activityText: string;
        let titleText: string;
        if (isSub) {
          const sub = subagentCharacters.find((s) => s.id === id);
          titleText = sub ? sub.label : 'Subtask';
          if (subHasPermission) {
            activityText = 'Needs approval';
          } else {
            activityText = titleText;
          }
        } else if (isCopilot) {
          titleText = 'Copilot Chat';
          activityText = 'Copilot Chat';
        } else if (isUser) {
          const remoteName = remoteUserNames.get(id);
          titleText = remoteName ?? 'You';
          const userActivity = getActivityText(id, agentTools, ch.isActive);
        ...
        const detailActivityText = activityText;
