  ...
  for (const item of furniture) {
    ...
    for (let dr = 0; dr < entry.footprintH; dr++) {
      for (let dc = 0; dc < entry.footprintW; dc++) {
        ...
        // Sofas/couches are never desk seats, regardless of adjacency
        const typeLower = item.type.toLowerCase();
        const isLoungeFurniture = typeLower.includes('sofa') || typeLower.includes('couch') || typeLower.includes('bench');
        const facingDelta = dirs.find((d) => d.facing === facingDir);
        const nearDesk = !isLoungeFurniture && !!facingDelta &&
          deskTiles.has(`${tileCol + facingDelta.dc},${tileRow + facingDelta.dr}`);
        console.log(`[layoutToSeats] type=${item.type} isLounge=${isLoungeFurniture} nearDesk=${nearDesk}`);
