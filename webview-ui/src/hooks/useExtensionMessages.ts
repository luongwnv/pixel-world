...
export function useExtensionMessages(
  ...
  const copilotAgentIdsRef = useRef(new Set<number>());
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        if (copilotAgentIdsRef.current.has(id)) {
          copilotAgentIdsRef.current.delete(id);
          setCopilotAgentIds(new Set(copilotAgentIdsRef.current));
        }
        ...
        os.clearPermissionBubble(id);
      } else if (msg.type === 'userActivityChange') {
        // Reassign user character to appropriate seat and walk there
        const id = msg.id as number;
        const preferDesk = msg.preferDesk as boolean;
        os.reassignUserSeat(id, preferDesk);
        ...
        if (status === 'waiting' && !userAgentIdsRef.current.has(id)) {
