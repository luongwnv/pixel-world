...
export async function launchNewTerminal(
  ...
  const configuredModel = vscode.workspace.getConfiguration('pixelAgents').get<string>('claudeModel', '');
  const modelFlag = configuredModel ? ` --model ${configuredModel}` : '';
  terminal.sendText(`claude --session-id ${sessionId}${modelFlag}`);
...
export function createPendingClaudeAgent(
  nextAgentIdRef: { current: number },
  nextTerminalIndexRef: { current: number },
  agents: Map<number, AgentState>,
  activeAgentIdRef: { current: number | null },
  ...
  agents.set(id, agent);
  activeAgentIdRef.current = id;
  console.log(`[Pixel Agents] Agent ${id}: pending (waiting for Claude Code session)`);
  webview?.postMessage({ type: 'agentCreated', id, folderName });
...
export function persistAgents(
  ...
  for (const agent of agents.values()) {
    // Only persist agents with a terminal (panel-based agents can't be restored)
