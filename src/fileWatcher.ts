  ...
  for (const file of files) {
    if (!knownJsonlFiles.has(file)) {
      ...
      if (activeAgentIdRef.current !== null) {
        // Active agent focused → /clear reassignment or pending agent adoption
        console.log(
        ...
        // Check for a pending agent (no JSONL file yet) first
        ...
        if (pendingId !== null) {
          console.log(
            `[Pixel Agents] New JSONL detected: ${path.basename(file)}, adopting to pending agent ${pendingId}`,
          );
          reassignAgentToFile(
            pendingId,
            file,
